...
- 靜態版依賴 cors.eu.org 免費代理服務。官網庫存 API 只接受 POST，僅支援 GET 的代理（allorigins、codetabs 等）無法使用，
  corsproxy.io、corsfix 則已改為需註冊網域，因此目前可用的公共代理僅此一家
- 靜態版已內建逾時（15 秒）、自動重試（每家代理 3 次、指數退避）與代理清單容錯（`PROXIES` 陣列，日後可加新代理）；
  查詢失敗時會明確區分「代理服務異常」與「官網異常」，並在代理全掛時提示改用本機伺服器版
- 代理失效期間請改用本機伺服器版（`node server.js`），該版不經第三方代理
